function [] = logistic_regression(trainData, degree, testData)
   % implement logistic regression using Iterative Reweighted Least Square
   % 
   % 
   % Inputs:
   %       train_data: column is a random variable, rows are observations,
   %                   last colum is the class value = [0 to classnum-1]      
   %                       
   %
   %       degree:     The second argument, <degree> is a number equal to either 1 or 2. 
   %                   We will not test your code with any other values. The degree specifies what function ¦Õ you should use. 
   %                   Suppose that you have an input vector x = (x1, x2, ..., xD)T,.
   %                   If the degree is 1, then ¦Õ(x) = (1, x1, x2, ..., xD)T.
   %                   If the number is 2, then ¦Õ(x) = (1, x1, (x1)2, x2, (x2)2..., xD, (xD)2)T.
   % 
   %       testData:   column is a random variable, rows are observations,
   %                   last colum is the class value = [0 to classnum-1] 
